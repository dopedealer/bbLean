#ifndef _BBCORE_BBSENDDATA_H
#define _BBCORE_BBSENDDATA_H
...
template <class T_ = void> // dummy 'template' to allow multiple includes without conflicts
BOOL BBSendData(
    HWND hwnd,          // window
    ...
    )
{
    ...
    if (::IsWow64Message())
        data -= 4, size -= 4;
    // dbg_printf("1 IsWow64Message: %d %d", IsWow64Message(), size);
#endif
...
UINT BBReceiveData(
        HWND hwnd,
        LPARAM lParam,
        int (*fn)( HWND hwnd, UINT msg, WPARAM wParam, const void *data, unsigned data_size)
        )
{
    bb_senddata* pBBSD;
    UINT msg;
    ...
    pBBSD = (bb_senddata*)((COPYDATASTRUCT*)lParam)->lpData;
    wParam = pBBSD->wParam;
    data = pBBSD->lParam_data;
    ...
    // dbg_printf("2 IsWow64Message: %d %d %x", IsWow64Message(), size, msg);
#endif
