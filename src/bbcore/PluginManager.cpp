...
// Privat variables
static plugins* bbplugins; // list of plugins
static HWND hSlit;  // BBSlit window
static FILETIME rc_filetime; // plugins.rc filetime

// Forward decls
static void apply_plugin_state(void);
static int load_plugin(plugins* q, HWND hSlit);
static int unload_plugin(plugins* q);
static void free_plugin(plugins** q);
static plugins* append_plugin(const char *rcline);
static bool write_plugins(void);
static void plugin_error(plugins* q, int error);
...
{
    return 0 != diff_filetime(bbPluginsRcPath(NULL), &rc_filetime);
}

//===========================================================================
// API(?candidate): EnumPlugins
...
static
plugins* append_plugin(const char *rcline)
{
...
static
void plugin_error(plugins* q, int error)
{
    const char *msg;
    switch (error)
    {
...
static
int load_plugin(plugins* q, HWND hSlit)
{
...
static
Menu *get_menu(const char *title, char *menu_id, bool pop, plugins** qp, bool b_slit)
{
    plugins* q;
    char *end_id;
    [[maybe_unused]] MenuItem* pItem{}; 
...
int PluginManager_handleBroam(const char *args)
{
    ...
    if (q) {
        ...
        if (e_remove == action || (e_add == action && false == q->enabled))
            free_plugin((plugins**)member_ptr(&bbplugins, q));
