...
/// \brief Get a pointer to core's static ToolbarInfo
...
static const cfgmenu tb_sub_placement[] = {
...
static const cfgmenu tb_sub_settings[] = {
...
int beginToolbar(HINSTANCE hi)
{
    ...
    memset(&gTBInfo, 0, sizeof gTBInfo);
...
void Toolbar_set_pos(void)
{
    ...
    SetWindowPos(Toolbar_hwnd,
        gTBInfo.onTop ? HWND_TOPMOST : HWND_NOTOPMOST,
        gTBInfo.xpos,
        gTBInfo.autohidden ? Toolbar_HideY : gTBInfo.ypos,
        gTBInfo.width,
        gTBInfo.height,
        Toolbar_hidden
    ...
    if (gTBInfo.bbsb_hwnd)
        PostMessage(gTBInfo.bbsb_hwnd, BB_TOOLBARUPDATE, 0, 0);
}
...
void Toolbar_button_event(UINT message, LPARAM i)
{
    ...
    if (WM_LBUTTONUP == message) {
        ...
        if (gTBInfo.bbsb_hwnd)
            co = gTBInfo.bbsb_currentOnly, rt = gTBInfo.bbsb_reverseTasks;

        co ^= (0 == (GetAsyncKeyState(VK_SHIFT) & 0x8000));
        switch(i)
        {
            case 0:
                PostMessage(gBBhwnd, BB_WORKSPACE, BBWS_DESKLEFT, 0);
                break;
            case 1:
                PostMessage(gBBhwnd, BB_WORKSPACE, BBWS_DESKRIGHT, 0);
                break;
            case 2:
                PostMessage(gBBhwnd, BB_WORKSPACE, (int)rt^BBWS_PREVWINDOW, co);
                break;
            case 3:
                PostMessage(gBBhwnd, BB_WORKSPACE, (int)rt^BBWS_NEXTWINDOW, co);
                break;
        }
    }
}
...
{
    RECT s = {0,0,0,0};
    bbDrawTextImpl(hdc, cp, &s, DT_CALCRECT|DT_NOPREFIX, 0);
    return s.right;
}
...
void PaintToolbar(HDC hdc, RECT *rcPaint)
{
    ...
    r.right = r.bottom = tbButtonWH;
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            bbDrawPixImpl(buf, &btn->r, pSI->picColor, (i&1) ? BS_TRIANGLE : -BS_TRIANGLE);
        }
    ...
    bbDrawTextImpl(buf, Toolbar_WorkspaceName, &r, justify, pSI->TextColor);
    ...
    r.left  += tbLabelIndent;
    r.right -= tbLabelIndent;
    bbDrawTextImpl(buf, Toolbar_CurrentWindow, &r, justify, pSI->TextColor);
    ...
    r.left  += tbLabelIndent;
    r.right -= tbLabelIndent;
    bbDrawTextImpl(buf, Toolbar_CurrentTime, &r, justify, pSI->TextColor);
...
static LRESULT CALLBACK Toolbar_WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        //====================
        case WM_CREATE:
            gTBInfo.hwnd = Toolbar_hwnd = hwnd;
        ...
        case WM_DESTROY:
            ...
            gTBInfo.hwnd = Toolbar_hwnd = NULL;
            break;
        ...
        if (wParam == TOOLBAR_AUTOHIDE_TIMER)
        {
            if (gTBInfo.autoHide)
            {
                if (check_mouse(hwnd) || (gTBInfo.bbsb_hwnd && check_mouse(gTBInfo.bbsb_hwnd)))
                    break;
        ...
        case WM_RBUTTONUP:
        {
            ...
            if (x < tbClockW)
                PostMessage(gBBhwnd, BB_MENU, BB_MENU_TASKS, 0);
            else
            if (x >= gTBInfo.width - tbClockW)
                PostMessage(gBBhwnd, BB_MENU, BB_MENU_ROOT, 0);
            else
        ...
        case WM_MBUTTONUP:
            ...
            if (wParam & MK_SHIFT)
                PostMessage(gBBhwnd, BB_TOGGLEPLUGINS, 0, 0);
            else
                PostMessage(gBBhwnd, BB_TOGGLETRAY, 0, 0);
        break;
...
void Toolbar_UpdatePosition(void)
{
    ...
    toolbar_setting* tbs = &Settings_toolbar;

    // Another Toolbar has set the info:
    if (NULL == Toolbar_hwnd && gTBInfo.hwnd)
        return;
    ...
    gTBInfo.placement        = place;
    gTBInfo.widthPercent     = tbs->widthPercent;
    gTBInfo.height           = tbheight;
    gTBInfo.width            = tbwidth;
    gTBInfo.ypos             = (place<3) ? 0 : ScreenHeight - gTBInfo.height;
    gTBInfo.xpos             = (place%3) * (ScreenWidth - gTBInfo.width) / 2;

    gTBInfo.onTop            = tbs->onTop;
    gTBInfo.autoHide         = tbs->autoHide;
    gTBInfo.pluginToggle     = tbs->pluginToggle;
    gTBInfo.autohidden       = gTBInfo.autoHide;
    gTBInfo.alphaValue       = (BYTE)tbs->alphaValue;
    gTBInfo.transparency     = tbs->alphaEnabled;
    gTBInfo.disabled         = false == tbs->enabled;
    gTBInfo.hidden           = false == tbs->enabled;

    if (Toolbar_hwnd)
    {
        int d = gTBInfo.height - 1;
        Toolbar_HideY = gTBInfo.ypos + (place < 3 ? -d : d);

        setDesktopMargin(
            Toolbar_hwnd,
            place < 3 ? BB_DM_TOP : BB_DM_BOTTOM,
            gTBInfo.autoHide || Toolbar_hidden ? 0 : gTBInfo.height);
...
{
    if (gTBInfo.autohidden != hidden)
    {
        gTBInfo.autohidden = hidden;
        Toolbar_set_pos();
    }
} 
