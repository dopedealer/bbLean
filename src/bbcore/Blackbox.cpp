...
const char* bbPath(const char* new_name, char* path, const char* default_name)
{
    if (new_name)
    {
...
{
    return bbPath(other, bbBlackboxrcBuffer(), "blackbox");
}

/// \returns The handle to the extensions.rc file that is being used
const char* bbExtensionsRcPath(const char* other)
{
    return bbPath(other, bbExtensionRcBuffer(), "extensions");
}

/// \returns The handle to the menu file that is being used
const char* bbMenuPath(const char* other)
{
    return bbPath(other, bbMenuRcBuffer(), "menu");
} 

/// \returns The handle to the plugins rc file that is being used
const char* bbPluginsRcPath(const char* other)
{
    return bbPath(other, bbPluginrcBuffer(), "plugins");
}

/// \brief The handle to the style file that is being used
const char* bbStylePath(const char* other)
{
    return bbPath(other, bbStyleRcBuffer(), NULL);
} 
...
static const proc_info rtl_list[] =
{
...
{
    const proc_info* rtl_ptr = rtl_list;
    do {
...
bool check_options (const char* lpCmdLine)
{
    while (*lpCmdLine)
    {
        ...
        if (option[0] == '-')
            ...
            case 5: // -toggle
                if (gBBhwnd) {
                    PostMessage(gBBhwnd, BB_QUIT, 0, BBOPT_QUIET);
                    return true;
                }
                nostartup = true;
                continue;
            ...
            case 6: // -exec <broam>
                if (gBBhwnd) {
                    ...
                    BBSendData(gBBhwnd, BB_EXECUTE, 0, lpCmdLine, -1);
                }
                return true;
        }
...
Blackbox::Blackbox(HINSTANCE hinst, LPSTR lpCmdLine)
{
    ...
    gSettingBbox = &settings; // TODO: remove global in future
}
...
{ 
    ...
    for (;;)
    {
        ...
        rc = bbMessageBox(MB_YESNO, NLS2("$Error_StartedTwice$",
                "Blackbox already running!"
                "\nDo you want to close the other instance?"
                ));
        if (IDYES != rc)
            return 0;
        SendMessage(gBBhwnd, BB_QUIT, 0, BBOPT_QUIET);
        BBSleep(1000);
        nostartup = true;
    }
    ...
    if (false == nostartup && (0x8000 & GetKeyState(VK_CONTROL)))
    {
        ...
        rc = bbMessageBox(MB_YESNO, NLS2("$Query_Escape$",
    ...
    init_runtime_libs();
    bbSetMultimon((bool)&::GetMonitorInfoA);
    /* Are we running on top of Explorer? */
    gUnderExplorer = NULL != FindWindow("Shell_TrayWnd", NULL);
    nostartup |= gUnderExplorer || StartupHasBeenRun();
    ...
    if (false == gUnderExplorer && have_imp(pSetProcessShutdownParameters))
...
void startup_blackbox(void)
{
    ...
    gSettingBbox->readStyleSettings(); 
    ...
    BBSleep(0);
    terminate_welcomescreen();
    SetTimer(gBBhwnd, BB_RUNSTARTUP_TIMER, 2000, NULL);
}
...
void set_misc_options(void)
{
    ...
    p = read_string(bbExtensionsRcPath(NULL), "blackbox.options.locale:", "");
...
void set_style(const char* filename)
{
    ...
    {
        write_string(bbRcPath(NULL), "session.styleFile",
...
LRESULT CALLBACK MainWndProc (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        do_restart:
            ...
            if (str) {
                write_string(bbExtensionsRcPath(NULL), "blackbox.theme:", str);
                m_free((char*)str);
            }

            register_fonts();
            gSettingsMenu.showBroams = false;
        ...
        case BB_GETSTYLE:
            return BBSendData((HWND)lParam, BB_SENDDATA, wParam, bbStylePath(NULL), -1);
        ...
        case WM_TIMER:
            ...
            switch (wParam) {
                ...
                case BB_ENDSTARTUP_TIMER:
                    exec_command(read_string(bbExtensionsRcPath(NULL), "blackbox.startup.run", NULL));
                    break;
...
void about_style(void)
{
    ...
    bbMessageBox(MB_OK,
        ...
        read_string(cp, "style.name",
            NLS2("$About_Style_NoName$", "[Style name not specified]")),
        NLS2("$About_Style_By$", "by"),
        read_string(cp, "style.author",
            NLS2("$About_Style_NoAuthor$", "[Author not specified]")),
        read_string(cp, "style.date", ""),
        read_string(cp, "style.credits", ""),
        read_string(cp, "style.comments", "")
        );
}
...
void edit_file(int id, const char* path)
{
    ...
    switch (id)
    {
        case 0:  path = bbStylePath(NULL);        goto edit;
        case 1:  path = bbMenuPath(NULL);         goto edit;
        case 2:  path = bbPluginsRcPath(NULL);       goto edit;
        case 3:  path = bbExtensionsRcPath(NULL); goto edit;
...
void exec_command(const char *cmd)
{
    ...
    if ('@' == cmd[0])
    {
        SendMessage(gBBhwnd, BB_BROADCAST, 0, (LPARAM)cmd);
    }
    else
    {
...
void post_command_fmt(const char *fmt, ...)
{
    ...
    PostMessage(gBBhwnd, BB_POSTSTRING, 0, (LPARAM)m_formatv(fmt, arg_list));
    va_end(arg_list);
}
...
static_assert(e_mask > e_bool); // 'modify' flags with cases must not overlap
...
int exec_core_broam(const char *broam)
{
    ...
    else if (action->flag & e_lpnum)
    {
        lParam = atoi(core_args)-1;
    }
    else if (action->flag & e_lptask)
    {
    ...
    switch (action->flag & e_mask)
    {
        ...
        case e_rootCommand:
            ...
            PostMessage(gBBhwnd, BB_MENU, BB_MENU_UPDATE, 0);
            break;
    ...
    if (msg) {
        ...
        if (action->flag & e_post)
            PostMessage(gBBhwnd, msg, wParam, lParam);
        else
            SendMessage(gBBhwnd, msg, wParam, lParam);
    }
    ...
    switch ((size_t)mode)
    {
        ...
        case BBSD_LOGOFF:
            if (ExitWindowsEx(EWX_LOGOFF, 0))
            {
                if (false == usingNT)
                {
                    PostMessage(gBBhwnd, WM_QUIT, 0, 0);
                }
                return 0;
            }
            break;
    ...
    switch (mode)
    {
        ...
        case BBSD_EXITWIN: // Standard Windows shutdown menu
            if (have_imp(pMSWinShutdown))
            {
                if (usingVista)
                {
                    return ((BOOL(WINAPI*)(HWND, int))pMSWinShutdown)(gBBhwnd, 0);
                }
                else
                {
                    return ((BOOL(WINAPI*)(HWND))pMSWinShutdown)(gBBhwnd);
                }
            }
            return 0;
    }
...
DWORD WINAPI RunStartupThread (void *pv)
{
    ...
    SetTimer(gBBhwnd, BB_ENDSTARTUP_TIMER, 100, NULL);
    return 0;
}
...
static void reset_rcpaths(void)
{
    ...
    bbResetBlackboxrcPath();
    bbResetExtensionRcPath();
    bbResetMenuRcPath();
    bbResetPluginRcPath();
    bbResetStyleRcPath();
} 
