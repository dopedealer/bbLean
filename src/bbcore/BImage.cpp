...
/// \brief Creates a gradient and fills it with the specified options
/// \param hdc Handle of the device context to fill
/// \param rect Rect you wish to fill
/// \param type Type of gradient (eg. B_HORIZONTAL)
/// \param Color First color to use
/// \param ColorTo Second color to use
/// \param interlaced Whether to interlace (darken every other line)
/// \param bevelstyle Style of bevel (eg. BEVEL_FLAT)
/// \param bevelposition Position of bevel (eg. BEVEL1)
/// \param bevelWidth Not used
/// \param borderColor Border color
/// \param borderWidth Width of border around bitmap
void makeGradient(
    HDC hdc,
    RECT rect,
    int type,
...
void setup_bmiHeader(bimage* bi, BITMAPINFOHEADER* bmiHeader)
{
...
void copy_to_hdc(bimage* bi, HDC hdc, int px, int py, int w, int h)
{
...
/// \brief Make a gradient from style Item
...
static void TrueColorDither(bimage* bi)
{
...
static void make_delta_table(bimage* bi, int *m, bool inv)
{
    int i = 0;
    do {
...
inline static void lighter(bimage* bi, unsigned char *pixel)
{
...
inline static void darker(bimage* bi, unsigned char *pixel)
{
...
static void bevel(bimage* bi, bool sunken, int pos)
{
    int w, h, nx, ny;
...
inline static void table_fn(bimage* bi, unsigned char *p, int length, bool invert)
{
    unsigned char *c = p;
    int i, e, d;
...
inline static void diag_fn(bimage* bi, unsigned char *c, int x, int y)
{
...
inline static void rect_fn(bimage* bi, unsigned char *c, int x, int y)
{
...
inline static void elli_fn(bimage* bi, unsigned char *c, int x, int y)
{
...
// -------------------------------------
bimage* bimage_create(int width, int height,  StyleItem *si)
{
    int byte_size;
    int table_size;
    ...
    bi = (bimage*)malloc(
        sizeof(bimage)-1 + byte_size + table_size * BBP
        );
...
static HBITMAP create_bmp(bimage* bi)
{
    BITMAPINFOHEADER bmiHeader;
    HBITMAP bmp;
    void *p = NULL;
...
BYTE *bimage_getpixels(bimage* bi)
{
    return bi ? bi->pixels : NULL;
}

void bimage_destroy(bimage* bi)
{
    free(bi);
}
...
//===========================================================================
// API(?candidate): MakeGradientBitmap
//===========================================================================
