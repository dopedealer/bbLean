...
bool convert_string(char *dest, const void *src, int nmax, bool is_unicode)
{
    ...
    if (is_unicode)
    {
        bbWideCharToMbyte((const WCHAR*)src, buffer, sizeof buffer);
        src = buffer;
    }
...
{
    NOTIFYICONIDENTIFIER_MSGV1* s = (NOTIFYICONIDENTIFIER_MSGV1*)data;
...
DWORD WINAPI SSO_Thread(void *pv)
{
    ...
    else if (ERROR_SUCCESS == RegOpenKeyEx(hk0, key, 0, KEY_READ, &hk1))
    {
        int index;
        for (index = 0; ; ++index) {
...
void Tray_SetEncoding(void)
{
    ...
    if (utf8 != tray_utf8)
        dolist (p, trayIconList) {
            ...
            bbWideCharToMbyte(wstr, p->t.szTip, sizeof p->t.szTip);
        }
    tray_utf8 = utf8;
}
