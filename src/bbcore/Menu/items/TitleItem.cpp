...
void TitleItem::Paint(HDC hDC)
{
    ...
    int indent = gMenuInfo.nTitleIndent;
    ...
    bbDrawTextImpl(hDC, GetDisplayString(), &rect, justify, pSI->TextColor);
}
...
void TitleItem::Mouse(HWND hwnd, UINT uMsg, DWORD wParam, DWORD lParam)
{
    ...
    case WM_MBUTTONUP:
        if (m_bActive)
        {
            if (wParam & MK_SHIFT)
            {
                if (p->getPidlList())
                {
                    ShowContextMenu(NULL, first_pidl(p->getPidlList()));
                }
            }
            else
            {
    ...
    case WM_LBUTTONDBLCLK:
        p->setIsOnTop(false == p->getIsOnTop());
        p->SetZPos();
        break;
    ...
    case WM_LBUTTONDOWN:
        ...
        for (;;) {
            p->SetZPos();
            if (p->getIsOnTop() || window_under_mouse() == p->getWindowHandle())
                break;
