...
int_items[] =
{
    { &gSettingsMenu.mouseWheelFactor       ,   1,    10, -2  },
    { &gSettingsMenu.popupDelay             ,   0,   400, -2  },
    { &gSettingsMenu.maxWidth               , 100,   600, -2  },
    { &gSettingsMenu.alphaValue             ,   0,   255, 255 },
...
static const int_item* get_int_item(const void *v)
{
    const int_item* p = int_items;
...
static const cfgmenu cfg_sub_plugins[] = {
...
static const cfgmenu cfg_sub_opendirection[] = {
    { NLS0("Bullet"),               "menu.openDirection bullet",    gSettingsMenu.openDirection },
    { NLS0("Left"),                 "menu.openDirection left",      gSettingsMenu.openDirection },
    { NLS0("Right"),                "menu.openDirection right",     gSettingsMenu.openDirection },
    { NULL,NULL,NULL }
};

static const cfgmenu cfg_sub_menu_files[] = {
    { NLS0("Sort By Extension"),    "menu.sortByExtension", &gSettingsMenu.sortByExtension  },
    { NLS0("Show Hidden Files"),    "menu.showHiddenFiles", &gSettingsMenu.showHiddenFiles  },
    { NULL,NULL,NULL }
};


static const cfgmenu cfg_sub_menu[] = {
    { NLS0("Open Direction"),        NULL, cfg_sub_opendirection },
    { NLS0("Maximal Width"),        "menu.maxWidth",        &gSettingsMenu.maxWidth  },
    { NLS0("Popup Delay"),          "menu.popupDelay",      &gSettingsMenu.popupDelay  },
    { NLS0("Wheel Factor"),         "menu.mouseWheelFactor", &gSettingsMenu.mouseWheelFactor  },
    { NLS0("Files"),                NULL, cfg_sub_menu_files },
    { "", NULL, NULL },
    { NLS0("Transparency"),         "menu.alpha.Enabled",   &gSettingsMenu.alphaEnabled  },
    { NLS0("Alpha Value"),          "menu.alpha.Value",     &gSettingsMenu.alphaValue  },
    { NLS0("Draw Separators"),      "menu.drawSeparators",  &gSettingsMenu.drawSeparators  },
    { NLS0("Drop Shadows"),         "menu.dropShadows",     &gSettingsMenu.dropShadows },
    { "", NULL, NULL },
    { NLS0("Always On Top"),        "menu.onTop",           &gSettingsMenu.onTop  },
    { NLS0("Snap To Edges"),        "menu.snapWindow",      &gSettingsMenu.snapWindow  },
    { NLS0("On All Workspaces"),    "menu.sticky",          &gSettingsMenu.sticky  },
    { NLS0("Toggle With Plugins"),  "menu.pluginToggle",    &gSettingsMenu.pluginToggle  },
    { "", NULL, NULL },
    { NLS0("Show Bro@ms"),          "menu.showBro@ms",      &gSettingsMenu.showBroams },
    { NULL,NULL,NULL }
};

static const cfgmenu cfg_sub_graphics[] = {
...
static const cfgmenu cfg_sub_dm[] = {
...
static const cfgmenu cfg_sub_focusmodel[] = {
...
static const cfgmenu cfg_sub_snap[] = {
...
static const cfgmenu cfg_sub_workspace[] = {
...
static const cfgmenu cfg_sub_misc[] = {
...
Menu* CfgMenuMaker(const char *title, const char *defbroam, const cfgmenu* pm, bool pop, char *menu_id)
{
    ...
    for (;pm->text; ++pm)
    {
        ...
        if (cmd)
        {
            ...
            {
                pItem = makeMenuItemInt( pMenu, item_text, cmd, *iip->v, iip->minval, iip->maxval);
                if (-2 != iip->offval)
            ...
            disabled = (v == &Settings_styleXPFix && Settings_altMethod)
                    || (v == &gSettingsMenu.dropShadows && false == usingXP)
                    ;
        ...
        else if (v)
        {
            ...
            if ((DWORD_PTR)v <= SUB_PLUGIN_SLIT)
                ...
                pSub = CfgMenuMaker(item_text, defbroam, (cfgmenu*)v, pop, menu_id);
            if (NULL == pSub)
                continue;
...
static const cfgmenu* find_cfg_item(
    const char *cmd,
    const cfgmenu* pmenu,
...
const void *exec_internal_broam(
    const char *arg,
    const cfgmenu* menu_root,
    const cfgmenu** p_menu,
    const cfgmenu** p_item)
{
    const void *v = NULL;
...
int exec_cfg_command(const char *argument)
{
    ...
    if (p_menu == cfg_sub_graphics || v == &Settings_UTF8Encoding)
        PostMessage(gBBhwnd, BB_RECONFIGURE, 0, 0); // full reconfigure
    else
        PostMessage(gBBhwnd, BB_RECONFIGURE, 1, 0); // bypass plugins
